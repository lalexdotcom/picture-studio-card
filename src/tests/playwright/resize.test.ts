...
/**
 * The bug the handles exposed: on a live camera hui-image's `.ratio` stylesheet
 * names only `.ratio img` and `.ratio div`, so `ha-camera-stream` resolves
 * `height: 100%` against `height: 0` and is drawn as overflow. Every box we
 * compute — outline, halo, handles — was tracing a container that did not match
 * the drawn picture. Measured on frontend 20260729.6: stream 358.8 × 0, wrapper
 * 358.8 × 201.8.
 *
 * RED verification: run this test against the code before the injection in
 * PictureStudioImage.updated() and it fails — streamRect.height is 0.
 */
it("on a live camera, the stream fills the same box the wrapper traces", async () => {
  ...
  const hui = image.shadowRoot?.querySelector("hui-image") as HTMLElement | null;
  const stream = hui?.shadowRoot?.querySelector("ha-camera-stream") as HTMLElement | null;

  expect(stream).not.toBeNull();
  const streamRect = stream?.getBoundingClientRect();
  const wrapperRect = wrapper.getBoundingClientRect();
  expect(wrapperRect.height).toBeGreaterThan(0);
  expect(streamRect?.height).toBeCloseTo(wrapperRect.height, 1);
});
